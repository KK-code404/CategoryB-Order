...
def _shipment_out(db: Session, line: ShipmentLine) -> ShipmentLineOut:
    ...
    return ShipmentLineOut(
        ...
        order_no=line.order_no, material_no=line.material_no, part_no=line.part_no, product_name=line.product_name,
        quantity=line.quantity, receiver=line.receiver, phone=line.phone, address=line.address, remark=line.remark,
...
def dashboard(db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> DashboardOut:
    ...
    visible = [_shipment_out(db, line) for line in lines]
